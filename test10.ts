...
let sort_arr = kdt_invest.sort((a: number, b: number): number => {
  return b - a;
});
let NewSet = new Set(sort_arr); //Y축 중복제거
const Y_arr = [...NewSet]; //Y축 중복제거
const flexSet: flexSet = (ele, Direc, justify, align) => {
  ele.style.display = 'flex';
  ele.style.flexDirection = Direc;
  ele.style.justifyContent = justify;
  ele.style.alignItems = align;
};
const style: styles = (ele, width, height, color, padding, border) => {
